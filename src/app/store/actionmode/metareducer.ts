  ...
  return (state: AppState, action: actions.Actions) => {
    switch (action.type) {
      ...
      case actions.AUTO_FIX_CLICK: {
        const from = getActivePath(state, ActionSource.From);
        const to = getActivePath(state, ActionSource.To);
        const { fromPath, toPath } = AutoAwesome.autoFix(from, to);
        state = updateActivePathBlock(state, ActionSource.From, fromPath);
        state = updateActivePathBlock(state, ActionSource.To, toPath);
        break;
      }
